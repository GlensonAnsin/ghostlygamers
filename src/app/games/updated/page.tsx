...
import { fetchAllUpdatedGames } from "@/app/lib/gameApi";
...
function UpdatedGamesContent() {
    const router = useRouter();
    const searchParams = useSearchParams();
    const [updatedGames, setUpdatedGames] = useState<Game[]>([]);
    ...
    useEffect(() => {
        const loadUpdatedGames = async () => {
            try {
                const data = await fetchAllUpdatedGames(currentPage);
                setUpdatedGames(data.results);
    ...
    const updatePageUrl = (page: number) => {
        router.push(`/games/updated?page=${page}`);
    }
    ...
    return (
        <div className={styles.page}>
            ...
            <main className={loading? styles.mainLoading : styles.main}>
                ...
                {!loading && (
                    <>
                        <h2 className={styles.h2Header}>Recently Updated Games</h2>
                        <div className={styles.gamesGrid}>
                            {updatedGames.map((game: Game) => (
